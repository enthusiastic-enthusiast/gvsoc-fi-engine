...
if os.environ.get('USE_GVRUN2') is not None:
    ...
    class Component(SystemTreeNodeParameter):
        ...
        def __init__(self, parent: 'Component', name: str | None,
        ...
        def add_libraries(self, libs: list[str]):
            """Add link libraries.
            ...
            if self.component is None and len(self.sources) != 0:
                if len(self.variants) == 0:
                    ...
                    for variant_name, variant_flags in self.variants.items():
                        variants[variant_name] = get_generated_component(self.sources, self.c_flags + variant_flags, self.libs)
        ...
        def __expand_signature_bridges(self):
            ...
            def _is_io_v2_sig(sig):
                ...
                # which is the historic big-packet default (see
                # gvsoc.signature) and must get the same auto-crossing
                # treatment.
